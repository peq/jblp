...
/**
 * A uncompressed Blp image with a color palette and a index- and alpha list.
 *
 * @author Michael Mitteregger
 */
public class BlpUncompressed1 {
     ...
     */
    public BlpUncompressed1(final BlpHeader blpHeader, final InputStream inputStream) throws IOException {
    ...
    private void readBody(final InputStream inputStream) throws IOException {
        ...
        for (int currentMipMap = 0; currentMipMap < this.mipMaps.length; currentMipMap++) {
            mipMap = new MipMap();
            indexList = new short[this.blpHeader.getMipMapSize()[currentMipMap] / 2];
            alphaList = new short[this.blpHeader.getMipMapSize()[currentMipMap] / 2];

            for (int i = 0; i < this.blpHeader.getMipMapSize()[currentMipMap] / 2; i++) {
                indexList[i] = ledis.readUByte();
            }

            for (int i = 0; i < this.blpHeader.getMipMapSize()[currentMipMap] / 2; i++) {
                alphaList[i] = ledis.readUByte();
            }

            mipMap.setIndexList(indexList);
            mipMap.setAlphaList(alphaList);
    ...
    private void createBufferedImage() {
        ...
        for (int y = 0; y < h; y++) {
            for (int x = 0; x < w; x++) {
                ...
                Color colorWithTransparency = new Color(color.getRed(), color.getGreen(), color.getBlue(), //
                       mipMap.getAlphaList()[i]);
    ...
    /**
     * A MipMap with a index- and alpha list.
     *
     * @author Michael Mitteregger
     */
    private class MipMap {
        ...
        public short[] getAlphaList() {
            return alphaList;
        }

        public void setAlphaList(final short[] alphaList) {
